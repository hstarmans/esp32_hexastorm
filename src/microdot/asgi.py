        ...
        while True:
            ...
            if message["type"] == "lifespan.startup":
                ...
                try:
                    ...
                    await send({"type": "lifespan.shutdown.failed", "message": repr(e)})
                else:
    ...
    async def asgi_app(self, scope, receive, send):
        ...
        async def cancel_monitor():
            ...
            while True:
                event = await receive()
                if (
                    event is None or event["type"] == "http.disconnect"
                ):  # pragma: no cover
                    cancelled = True
                    break
        ...
        try:
            while not cancelled:  # pragma: no branch
                ...
                await send(
                    {"type": "http.response.body", "body": res_body, "more_body": True}
                )
...
class WebSocket(BaseWebSocket):  # pragma: no cover
    ...
    async def send(self, data):
        if isinstance(data, str):
            await self.request.sock[1]({"type": "websocket.send", "text": data})
        else:
            await self.request.sock[1]({"type": "websocket.send", "bytes": data})
...
async def websocket_upgrade(request):  # pragma: no cover
    ...
    ws = (
        WebSocket(request)
        if not request.app.embedded_server
        else BaseWebSocket(request)
    )
    await ws.handshake()
