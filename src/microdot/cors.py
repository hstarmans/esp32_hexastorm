class CORS:
    ...
    def __init__(
        ...
        self.allowed_headers = (
            None if allowed_headers is None else [h.lower() for h in allowed_headers]
        )
    ...
    def get_cors_headers(self, request):
        ...
        if self.allowed_origins == "*":
            ...
            cors_headers["Vary"] = "Origin"
        if self.allow_credentials and "Access-Control-Allow-Origin" in cors_headers:
        ...
        if request.method == "OPTIONS":
            ...
            if method:
                method = method.upper()
                if self.allowed_methods is None or method in self.allowed_methods:
            ...
            if headers:
                if self.allowed_headers is None:
                    ...
                    headers = [h for h in headers if h.lower() in self.allowed_headers]
    ...
    def after_request(self, request, response):
        ...
        if saved_vary and saved_vary != response.headers.get("Vary"):
            response.headers["Vary"] = saved_vary + ", " + response.headers["Vary"]
