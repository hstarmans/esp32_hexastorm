...
class TestClient:
    ...
    def _update_cookies(self, res):
        ...
        for cookie in cookies:
            ...
            for option in cookie_options[1:]:
                ...
                if option.startswith("max-age="):  # pragma: no cover
    ...
    async def request(self, method, path, headers=None, body=None, sock=None):
        ...
        req = await Request.create(
            self.app, reader, writer, ("127.0.0.1", 1234), scheme=self.scheme
        )
    ...
    async def websocket(self, path, client, headers=None):
        ...
        class FakeWebSocket:
            ...
            async def _next(self, data=None):
                ...
                opcode = WebSocket.TEXT if isinstance(data, str) else WebSocket.BINARY
        ...
        sock = FakeWebSocket()
        return await self.request("GET", path, headers=ws_headers, sock=(sock, sock))
