...
class Compiler:
    ...
    def parse_statement(self, stmt):
        ...
        if tokens[0] == "args":
            ...
            if tokens[0][0] == "{":
                ...
                self.file_out.write(
                    '_ = __import__(%s.replace(".", "_"), None, None, 1)\n'
                    % tokens[0][2:-2]
                )
                self.indent()
    ...
    def parse_line(self, line):
        while line:
            start = line.find(self.START_CHAR)
            if start == -1:
                self.literal(line)
                return
            ...
            # print("*%s=%s=" % (sel, EXPR))
            if sel == self.STMNT:
                end = line.find(self.STMNT_END)
                assert end > 0
                stmt = line[
                    start + len(self.START_CHAR + self.STMNT) : end
                ].strip()
                self.parse_statement(stmt)
                end += len(self.STMNT_END)
                line = line[end:]
                if not self.in_literal and line == "\n":
                    break
            elif sel == self.EXPR:
                #            print("EXPR")
                end = line.find(self.EXPR_END)
                assert end > 0
                expr = line[
                    start + len(self.START_CHAR + self.EXPR) : end
                ].strip()
