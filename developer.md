...
The next step depend on the exact chip family you use for the ESP32.
First the ESP32 needs to enter boot mode. Set the boot pin to low and toggle the reset pin.
Release the boot pin. The board should enter boot mode. This typically causes the device to connect to a different com port.
Note that COM9 implies /dev/ttyS9 in the linux WSL layer. 
